...
start.ps1              # Windows：主启动脚本（.venv、pip/npm 镜像、npm install、启动 server.py）
frontend/              # 前端源码
...
`start.ps1` 会创建 `.venv`、安装依赖与前端 `npm install`，最后执行 `python server.py`。
...
- `**start.sh` / `start.ps1`**：`start.sh` 在 `LANG` 为空或为 `C`/`POSIX` 时默认 `en_US.UTF-8`；Windows 主启动使用 `start.ps1`。
