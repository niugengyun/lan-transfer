...
终端会打印本机访问地址与检测到的**局域网 IP**；同网段设备使用 `http://<你的局域网IP>:8888` 访问主站（默认端口 **8888**）。本机管理页 `/admin` 可保存端口到 `data/server_settings.json`（重启后生效）；若设置了环境变量 `PORT`，则始终以 `PORT` 为准。
...
| 变量           | 默认值                                              | 含义                                              |
| ------------ | ------------------------------------------------ | ----------------------------------------------- |
| `PORT`       | （未设置时默认 **8888**，或见 `data/server_settings.json`） | 非空时优先作为 HTTP 监听端口                               |
| `HOST`       | `0.0.0.0`                                        | 绑定地址                                            |
